...
Future<void> exportedLog(
  ...
  print("LOG_EXPORT_CALL__$file_name}");
...
Future<void> exportedLogTest(
  ...
  final filePath =
      '${downloadsDirectory.path}/${file_name + "${DateTime.now().hour}_${DateTime.now().minute}_${DateTime.now().second}"}.txt';
  final file = File(filePath);
  try {
    await file.writeAsString(logMessage, mode: FileMode.append);
  } catch (e) {
...
oneSignalSettings() async {
  ...
  OneSignal.Notifications.addClickListener((notification) async {
    ...
    print("CHeckIdNotification::::${notId}");
